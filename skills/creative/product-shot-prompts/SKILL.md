...
- **Reference Mashups:** If a direct color match is missing in the library, use a "Mashup" technique: adopt the structure/composition of a luxury reference (e.g., R038's floating waves) and map a new custom color palette (e.g., Deep Cherry Red) to it. If the user requests a specific theme (e.g., "make it red" or "add cherries"), maintain structural fidelity but adapt the palette and props.
- **Human Presence Adaptation:** When adapting a still-life reference for a shot with human presence (e.g., holding a product), preserve the high-fidelity lighting and micro-details (dew, specular highlights) from the reference but shift the placement and surfaces to the human subject.
